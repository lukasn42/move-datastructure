...
class move_datastructure {
    ...
    /* number of remaining incoming edges after an output interval is cut, 2 <= b < a-1, (default = 2) */
    uint8_t b;
    ...
    /**
     * @brief checks if the an output interval is unbalanced
     * 
    ...
    /**
     * @brief balances an output interval and all by that newly created unbalanced output intervals starting before q_u
     * 
    ...
    public:
    /**
     * @brief creates move datastructure out of I by building L_in and T_out from I, balancing I and then building D_pair and D_index; in O(k log k)
     * 
    ...
    /**
     * @brief calculates the move query Move(I,i,x), in O(a)
     * 
     * @param i i in [0..n-1]
