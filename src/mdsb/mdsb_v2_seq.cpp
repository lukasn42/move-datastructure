...
void mdsb<T>::balance_v2_seq() {
    ...
    */
    ...
    // pointer to the pair (p_{i+b},q_{i+b})
    ...
    do {
        ...
        // If [q_j, q_j + d_j - 1] is unbalanced, insert ((p_{i+b},q_{i+b}),(p_j,q_j)) into T_e.
        if (pln_IpB != NULL) {
            it_inp.set(pln_IpB);
    ...
    pair_list_node<T> *pln_Ip2B,*pln_Z,*pln_ZpB;
    ...
    while (!T_e.empty()) {
        ...
        pln_Ip2B = is_unbalanced_seq(pln_IpB,ptn_NEW);
        ...
        if (p_j + d < q_j || q_j + d_j <= p_j + d) {
            ...
            */
            ...
            if (p_j + d < q_j || pln_Z->pr == NULL || pln_Z->pr->v.first < q_y) {
                pln_ZpB = is_unbalanced_seq(pln_Z,ptn_Y);
            }
        }
        ...
        if (pln_ZpB != NULL) {
            if (pln_Ip2B != NULL) {
                ...
                if (pln_ZpB->v.first < pln_Ip2B->v.first) {
                    ...
                    min->v = te_pair_seq<T>{pln_Ip2B,ptn_NEW};
                    T_e.insert_or_update_in(te_pair_seq<T>{pln_ZpB,ptn_Y},min);
                } else {
                    ...
                    if (T_e.size() == 1 || ptn_Y->v.v.second < T_e.second_smallest()->v.second->v.v.second) {
                        ...
                        min->v = te_pair_seq<T>{pln_ZpB,ptn_Y};
                        T_e.insert_node_in(new te_node_seq<T>(te_pair_seq<T>{pln_Ip2B,ptn_NEW}),min);
                    } else {
                        // and is not the second unbalanced output interval
                        min->v = te_pair_seq<T>{pln_Ip2B,ptn_NEW};
                        T_e.insert_or_update(te_pair_seq<T>{pln_ZpB,ptn_Y});
                    }
                }
            } else {
