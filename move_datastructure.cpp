...
void move_datastructure<T>::balance_upto(pair_list<T> *L_in, pair_tree<T> *T_out, pair_list_node<T>* nodePairI, pair_list_node<T>* nodePairJ, T q_u) {
    ...
    Case 1: If p_j + d_1 in [q_u, q_u + d_u - 1], then [q_u, q_u + d_u - 1] = [q_y, q_y + d_y - 1]. p_j + d_1 = q_u obviously is not possible.
            In general, once balance_upto creates a new input interval in [q_u, q_u + d_u - 1], the sequence of recursive calls ends.
...
move_datastructure<T>::move_datastructure(interv_seq<T> *I, T n, uint8_t a, uint8_t b) {
    ...
    do {
        ...
        // Find the next output interval with an incoming edge in the permutation graph and find the first input interval connected to it.
        do {
